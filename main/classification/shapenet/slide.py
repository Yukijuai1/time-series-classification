...
def slide_MTS_dim_step(X, class_label, alpha):
    ...
    add step to reduce the num of new TS
    '''
    num_of_old_MTS = numpy.shape(X)[0]  # 保存有几个数据
    dim_of_old_MTS = numpy.shape(X)[1]  # 每个数据的维度
    length_of_old_MTS = numpy.shape(X)[2]   # 每个数据的长度
    length_of_new_MTS = int(length_of_old_MTS * alpha)  # shaplet的长度
